...
export default function MovieDetails( props ) {
    ...
    const fetchData = async () => {
        const response = await fetch('http://localhost:4000/movies/' + props.selectedMovieId );
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        
        console.log('movie data fetched with id: ' + props.selectedMovieId)
        return response.json();
      }
      
    
      const {  isLoading, isError, data, error } = useQuery(['data', props.selectedMovieId], fetchData);
